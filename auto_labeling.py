...
class Model(nn.Module):
    ...
    def auto_label(self, source: Union[str, Path, int, list, tuple, np.ndarray, torch.Tensor], save_dir, **kwargs):
        results = self.predict(source, **kwargs)
        for result in results:
            img_name = Path(result.path).stem
            label_path = Path(save_dir) / f"{img_name}.txt"
            os.makedirs(os.path.dirname(label_path), exist_ok=True)
            with open(label_path, "w") as f:
                for box in result.boxes.data:
                    # Assuming box format is (class, x_center, y_center, width, height)
                    x1, y1, x2, y2, similarity, cls = box
                    x_center = (x1 + x2)/2/346
                    y_center = (y1 + y2)/2/260
                    width = (x2 - x1)/346
                    height = (y2 - y1)/260
                    if int(cls) != 0:
                        f.write(f"{int(cls)} {x_center:.6f} {y_center:.6f} {width:.6f} {height:.6f}\n")
                    # f.write(f"{int(cls)} {x_center:.6f} {y_center:.6f} {width:.6f} {height:.6f}\n")
        print(f"Labels saved to {save_dir}")
...
model = YOLO('/home/bcml1/WBC_project/ultralytics-Yolov8/runs/detect/train2/weights/best.pt')
model.auto_label(source='/home/bcml1/WBC_project/data/image/acc/no_14_acc20',
                  save_dir='/home/bcml1/WBC_project/test')